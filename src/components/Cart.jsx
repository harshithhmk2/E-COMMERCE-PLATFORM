...
const Cart = () => {
 ...
 useEffect(() => {
      ...
      const cartItemsWithImages = await Promise.all(
        cart.map(async (item) => {
          try {
            ...
            const response = await API.get(`/products/${item.id}/image`, { responseType: "blob" });
            const imageUrl = URL.createObjectURL(response.data);
            return { ...item, imageUrl };
          } catch (blobError) {
            console.warn(`Blob fetch failed for item ${item.id}, checking imageUrl field...`);
           
            return { ...item, imageUrl: item.imageUrl || "https://via.placeholder.com/150" };
          }
        })
      );
  ...
  if (cart.length > 0) fetchImagesAndUpdateCart();
  else setCartItems([]);
}, [cart]);
